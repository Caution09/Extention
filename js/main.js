...
class PromptGeneratorApp {
  ...
  setupContextMenuListener() {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      if (message.type === 'insertPrompt') {
        ...
        const targetElement = (activeElement && (activeElement.tagName === 'INPUT' || activeElement.tagName === 'TEXTAREA')) 
          ? activeElement 
  ...
  setupWindowHandlers() {
    $('#openWindow').on('click', () => {
      ...
      const message = displayType === 'page' 
        ? { type: 'openPage' }
  ...
  async doSearch(keyword, categories) {
    ...
    if (results.length > 0) {
      await this.listManager.createList('search', results, '#promptList', { isSave: false });
      $('#isSearch').html('');
    } else if (keyword) {
  ...
  async handleNoSearchResults(keyword) {
    ...
    translationPromises.push(
      this.translateWithService(keyword, 'Google', translateGoogle)
    ...
    if (AppState.userSettings.optionData?.deeplAuthKey) {
      translationPromises.push(
        this.translateWithService(keyword, 'DeepL', translateDeepl)
          .then(data => results.push(data))
      );
    }
    ...
    $('#isSearch').html('');
    await this.listManager.createList('search', results, '#promptList', { isSave: true });
  }
    ...
    return new Promise((resolve) => {
      translateFunc(keyword, (translatedText) => {
        ...
        const data = isAlphanumeric
          ? { prompt: keyword, data: { 0: '', 1: `${serviceName}翻訳`, 2: translatedText } }
          : { prompt: translatedText, data: { 0: '', 1: `${serviceName}翻訳`, 2: keyword } };
        resolve(data);
      });
    });
  }
  ...
  updateCategoryDropdown(targetId, categoryLevel, parentValue) {
    ...
    const categoryItems = categoryData.data[categoryLevel]
  ...
  async refreshEditList() {
    console.log('refreshEditList called, elements count:', editPrompt.elements.length);
    
    const editType = AppState.userSettings.optionData.editType;
    const listType = editType === CONSTANTS.EDIT_TYPES.SELECT ? 'editDropdown' : 'edit';
    
    // デバッグ: リストクリア前の状態を確認
    console.log('Before clear - list children:', $('#editList').children().length);
    
    await this.listManager.createList(listType, editPrompt.elements, '#editList');
    
    // デバッグ: リスト作成後の状態を確認
    console.log('After create - list children:', $('#editList').children().length);
  }
  ...
  toggleDictionary(type) {
    const configs = {
      prompt: {
        ...
        createFunc: () => this.listManager.createList('archive', AppState.data.archivesList, '#archiveList')
      },
      element: {
      ...
      master: {
        ...
        createFunc: () => this.listManager.createList('master', AppState.data.masterPrompts, '#masterDicList')
      }
    };
  ...
  async archivePrompt() {
    ...
    const validation = Validators.checkDuplicateArchive(prompt, AppState.data.archivesList);
  ...
  generatePrompt() {
    sendBackground('DOM', 'Generate', this.generateInput.val(), PositivePromptTextSelector, GenerateButtonSelector);
  }
  ...
  setupOptionHandlers() {
    $('#isDeleteCheck').on('change', (e) => {
      AppState.userSettings.optionData.isDeleteCheck = $(e.target).prop('checked');
      saveOptionData();
    });
  ...
  updateUIState() {
    ...
    if (AppState.data.searchCategory?.[0]) {
      ...
      this.updateCategoryDropdown('#search-cat1', 1, AppState.data.searchCategory[0]);
    ...
    if (AppState.userSettings.optionData?.shaping === 'NAI' && 
        PositivePromptTextSelector && GenerateButtonSelector) {
      $('#GeneratoButton').show();
    }
  }
}
...
class PromptListManager {
  constructor() {
    this.listConfigs = {
      search: {
        ...
        createItem: async ($li, item, index, options) => await this.createSearchItem($li, item, index, options)
      },
      add: {
        headers: ['大項目', '中項目', '小項目', 'Prompt'],
        createItem: async ($li, item, index, options) => await this.createAddItem($li, item, index, options),
        sortable: true
      },
      master: {
        headers: ['大項目', '中項目', '小項目', 'Prompt'],
        createItem: async ($li, item, index, options) => await this.createMasterItem($li, item, index, options)
      },
      archive: {
        headers: ['名前', 'Prompt'],
        createItem: async ($li, item, index, options) => await this.createArchiveItem($li, item, index, options),
        columnWidths: { 1: '150px' }
      },
      edit: {
        headers: ['Prompt', '重み'],
        createItem: async ($li, item, index, options) => await this.createEditItem($li, item, index, options),
        sortable: true,
        columnWidths: { 1: '400px', 2: '30px' }
      },
      editDropdown: {
        headers: ['大項目', '中項目', '小項目', 'Prompt', '重み'],
        createItem: async ($li, item, index, options) => await this.createEditDropdownItem($li, item, index, options),
        sortable: true,
        columnWidths: { 1: '80px', 2: '80px', 3: '80px', 4: '130px', 5: '30px' }
      }
    };
  }
  ...
  async createList(type, data, listId, options = {}) {
    ...
    console.log(`Creating ${type} list with ${data.length} items for ${listId}`);
    ...
    console.log(`List ${type} created, final children count:`, $(listId).children().length);
  }
  ...
  async createAddItem($li, item, index, options = {}) {
    // カテゴリー入力
    for (let i = 0; i < 3; i++) {
      $li.append(UIFactory.createInput({
        value: item.data[i],
        index: index,
        onInput: (value) => {
    ...
    const buttons = UIFactory.createButtonSet({
      ...
      copyValue: item.prompt,
      onDelete: async () => {
        AppState.data.localPromptList.splice(getLocalElementIndex(item), 1);
        await saveLocalList();
        $li.remove();
      }
    });
  ...
  async createEditDropdownItem($li, item, index, options = {}) {
    ...
    category = findCategory(AppState.data.masterPrompts) || findCategory(AppState.data.localPromptList);
    ...
    if (!category) {
      ...
      const element = AppState.data.masterPrompts.find(e => e.prompt === prompt);
      if (element?.url) {
    ...
    // カテゴリー連動設定
    this.setupCategoryDropdownChain(categoryInputs, $valueInput, $weightInput, index);
  ...
  setupCategoryDropdownChain(categoryInputs, $valueInput, $weightInput, index) {
    ...
    categoryInputs[2].attr('list', 'category' + categoryValue + categoryInputs[1].val());
    ...
    categoryInputs[1].on('change', function() {
      categoryInputs[2].attr('list', 'category' + categoryValue + $(this).val());
    });
    
    categoryInputs[2].on('change', function() {
      const inputValue = $(this).val();
      const masterPrompt = AppState.data.masterPrompts.find(p => p.data[2] === inputValue);
      const newPrompt = masterPrompt?.prompt || $valueInput.val();
      
      $valueInput.val(newPrompt);
      editPrompt.editingValue(editPrompt.getValue(AppState.userSettings.optionData.shaping, newPrompt, $weightInput.val()), index);
      window.app.updatePromptDisplay();
    });
  }
  ...
  queueTranslation(keyword, inputFields) {
    ...
    // 既にキューに存在する場合は追加しない
    const existing = this.translationQueue.find(item => item.keyword === keyword);
    if (existing) {
      return;
    }
    ...
    if (!this.translationTimer) {
      this.translationTimer = setTimeout(() => this.processTranslationQueue(), 100);
    }
  }
  ...
  async processTranslationQueue() {
    ...
    // 重複を除去
    const uniqueQueue = this.translationQueue.filter((item, index, self) =>
...
class FileHandler {
  ...
  addPromptDic(item) {
    const duplicate = AppState.data.archivesList.find(archive => archive.prompt === item.prompt);
    if (!duplicate) {
...
function archivesInit() {
  if (window.app && $('#archiveList').children().length > 0) {
    window.app.listManager.createList('archive', AppState.data.archivesList, '#archiveList');
  }
}
