  ...
  if (isHit) {
    ...
    if (keyword === "") {
      ...
      translateGoogle(keyword, (prompt) => {
        ...
        let data = {
          prompt: prompt,
          data: { 0: "", 1: "Google翻訳", 2: keyword },
        };
        const isAlphanumeric = /^[a-zA-Z0-9\s:]+$/.test(keyword);
        if (isAlphanumeric) {
          data = {
            prompt: keyword,
            data: { 0: "", 1: "Google翻訳", 2: prompt },
          };
        }
      ...
      if (optionData.deeplAuthKey) {
        ...
        translateDeepl(keyword, (prompt) => {
          ...
          let data = {
            prompt: prompt,
            data: { 0: "", 1: "Deepl翻訳", 2: keyword },
          };
          const isAlphanumeric = /^[a-zA-Z0-9\s:]+$/.test(keyword);
          if (isAlphanumeric) {
            data = {
              prompt: keyword,
              data: { 0: "", 1: "Deepl翻訳", 2: prompt },
            };
          }
          resultList.push(data);
          isSearch = isSearchGoogle || isSearchDeepl;
...
function archivesPrompt() {
  ...
  const matchedIndex = archivesList.findIndex(
    (obj) => obj.prompt === archivePrompt
  );

  if (matchedIndex !== -1) {
    window.alert(
      "既に同じプロンプトが追加されています。名前：" +
        archivesList[matchedIndex].title
    );
  } else {
  ...
  fetch(imageUrl)
    ...
    .then((blob) => {
      ...
      reader.onloadend = () => {
        ...
        $("#preview-positive-copy").click(() =>
          navigator.clipboard.writeText(value.prompt)
        );
        $("#preview-negative-copy").click(() =>
...
function createArchiveList(json, listId) {
  ...
  jsonLoop(json, function (item, index) {
    let li = $("<li>");
    li.append(
      createInputData(item.title, index, function (value, index) {
        archivesList[index].title = value;
        saveArchivesList();
      })
    );
    li.append(
      createInputData(item.prompt, index, function (value, index) {
...
function createEditDropdownList(json, listId) {
  ...
  jsonLoop(json, function (item, index) {
    ...
    inputData[0].on("change", function () {
      ...
      inputData[2].attr(
        "list",
        "category" + categoryValue + inputData[1].val()
      );
    });
    ...
    inputData[2].on("change", function () {
      ...
      const prompt =
        masterPrompts.find((value) => value.data[2] === inputValue)?.prompt ||
        valueInput.val();
      let element = masterPrompts.find((value) => value.prompt === prompt);
      if (element && element.url) {
      ...
      editPrompt.editingValue(
        editPrompt.getValue(optionData.shaping, prompt, weightInput.val()),
        index
      );
      UpdateGenaretePrompt();
    });
...
function setColumnWidth(listId, inputIndex, width) {
  $(listId)
    .find("li input:nth-of-type(" + inputIndex + ")")
    .css("width", width);
  $(listId)
    .find("ui input:nth-of-type(" + inputIndex + ")")
    .css("width", width);
}
  ...
  reader.onload = function (event) {
    ...
    switch (content.dicType) {
      case "Elements":
        ...
        if (addCount > 0) {
          window.alert(
            addCount.toString() + "件の要素辞書の読み込みが完了しました"
          );
        }
        break;
      case "Prompts":
        ...
        if (addCount > 0) {
          ...
          archivesInit();
          window.alert(
            addCount.toString() + "件のプロンプト辞書の読み込みが完了しました"
          );
        }
        break;
      case "Master":
...
function getTextChunk(arrayBuffer) {
  ...
  while (chunkOffset < arrayBuffer.byteLength) {
    ...
    if (chunkType === "tEXt") {
      ...
      const keywordArray = new Uint8Array(
        arrayBuffer,
        chunkOffset + 8,
        keywordEnd
      );
      const keyword = new TextDecoder().decode(keywordArray);
      const textDataArray = new Uint8Array(
        arrayBuffer,
